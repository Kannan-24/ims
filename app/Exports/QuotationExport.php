...
class QuotationExport implements FromCollection, WithHeadings, WithEvents, ShouldAutoSize
{
    ...
    public function __construct(Collection $quotations, $generatedAt, $dateRange)
    {
    ...
    public function collection()
    {
        if ($this->quotations->isEmpty()) {
            return collect([['No Quotations Found']]);
        }

        return $this->quotations->map(function ($quotation, $index) {
            return [
                '#' => $index + 1,
                'Quotation Code' => $quotation->quotation_code,
                'Quotation Date' => $quotation->quotation_date,
                'Customer Name' => $quotation->customer->company_name ?? 'N/A',
                'Customer GST' => $quotation->customer->gst_number ?? 'N/A',
                ...
                'GST' => number_format($quotation->gst, 2),
    ...
    {
        return $this->quotations->isEmpty() ? [] : [
            '#',
            'Quotation Number',
            'Quotation Date',
            'Customer Name',

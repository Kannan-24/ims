...
if (!isset($otpauth) && !empty($secret) && $user->two_factor_secret) {
    try {
        $otpauth = app(\App\Services\TotpService::class)->getOtpAuthUrl(
            config('app.name'),
            $user->email,
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        <!-- TOTP Method -->
        <div
            class="bg-white border border-gray-200 rounded-lg p-6 space-y-4 shadow-sm hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between mb-4">
                ...
                @if ($user->totp_enabled)
                    <span
                        class="flex items-center px-3 py-1.5 rounded-full bg-green-100 text-green-700 text-xs font-medium">
                        ENABLED
                    </span>
                @else
                    <span
                        class="flex items-center px-3 py-1.5 rounded-full bg-gray-100 text-gray-600 text-xs font-medium">
                        DISABLED
                    </span>
                @endif
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                @if (!$user->totp_enabled)
                    <button onclick="openModal('enable-totp-modal')"
                        class="w-full bg-blue-600 hover:bg-blue-700 px-4 py-3 rounded-lg text-white font-medium transition-colors">
                        Start TOTP Setup
                    </button>
                @endif
                ...
                @if ($user->preferred_2fa_method === 'totp' && !$user->two_factor_enabled && !empty($secret))
                    <div class="space-y-4 mt-4">
                        <div class="bg-white rounded-lg p-4 border border-gray-200">
                            ...
                            <div
                                class="bg-gray-50 p-3 rounded border border-gray-200 font-mono text-xs break-all text-gray-700 mb-4">
                                {{ $secret }}
                            </div>
                        ...
                        <form method="POST" action="{{ route('2fa.confirm') }}" class="space-y-3">
                            ...
                            <button
                                class="w-full bg-green-600 hover:bg-green-700 px-4 py-3 rounded-lg text-white font-medium transition-colors">
                                Confirm & Enable TOTP
                            </button>
                        </form>
                    </div>
        ...
        <div
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                @if (!$user->email_otp_enabled)
                    <button onclick="openModal('enable-email-otp-modal')"
                        class="w-full bg-blue-600 hover:bg-blue-700 px-4 py-3 rounded-lg text-white font-medium transition-colors">
                        Enable Email OTP
                    </button>
                @endif
                ...
                @if ($user->preferred_2fa_method === 'otp' && !$user->two_factor_enabled)
                    <form method="POST" action="{{ route('2fa.confirm') }}" class="space-y-3 mt-4">
                        ...
                        <div class="flex flex-col sm:flex-row gap-3">
                            <button
                                class="flex-1 bg-green-600 hover:bg-green-700 px-4 py-3 rounded-lg text-white font-medium transition-colors">
                                Confirm & Enable
                            </button>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-6 space-y-6 shadow-sm">
            <div class="text-center">
                ...
                <span
                    class="inline-flex items-center px-4 py-2 rounded-full text-sm font-semibold {{ $enabledCount ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200' }}">
                    @if ($enabledCount)
            ...
            <div class="grid grid-cols-2 gap-4">
                <div
                    class="bg-gray-50 rounded-lg p-4 border {{ $user->totp_enabled ? 'border-green-200' : 'border-gray-200' }}">
                ...
                <div
                    class="bg-gray-50 rounded-lg p-4 border {{ $user->email_otp_enabled ? 'border-green-200' : 'border-gray-200' }}">
                    <div class="flex items-center justify-between">
                        <div>
                        ...
                        <span
                            class="text-sm font-bold {{ $user->email_otp_enabled ? 'text-green-600' : 'text-gray-500' }}">
        ...
        <div class="bg-white rounded-lg w-full max-w-md p-6 space-y-4 shadow-lg">
            ...
            <form method="POST" action="{{ route('2fa.start') }}" class="space-y-4">
                ...
                <div class="flex gap-3">
                    <button
                        class="flex-1 bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg text-white text-sm font-medium transition-colors">
                        Generate QR Code
                    </button>
    ...
    <div id="disable-totp-modal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50">
        <div class="bg-white rounded-lg w-full max-w-md p-6 space-y-4 shadow-lg">
            ...
            <form method="POST" action="{{ route('2fa.method.disable') }}" class="space-y-4">
                ...
                <div class="flex gap-3">
                    <button
                        class="flex-1 bg-red-600 hover:bg-red-700 px-4 py-2 rounded-lg text-white text-sm font-medium transition-colors">
                        Disable
                    </button>
    ...
    <div id="disable-all-2fa-modal" class="hidden fixed inset-0 z-50 items-center justify-center bg-black/50">
        <div class="bg-white rounded-lg w-full max-w-lg p-6 space-y-4 shadow-lg">
            ...
            <form method="POST" action="{{ route('2fa.disable') }}" class="space-y-4" id="disable-all-2fa-form">
                ...
                <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                    <div class="flex items-start">
                        ...
                        <div class="text-sm text-red-800">
                            ...
                            <p>Disabling all 2FA methods will make your account less secure. Please verify with all
                                active methods.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    @if ($user->totp_enabled)
                        <div>
                            ...
                            <input type="text" name="totp_code" maxlength="6"
                                {{ $user->totp_enabled ? 'required' : '' }}
                                class="w-full border border-gray-300 rounded-lg p-3 text-center text-sm font-mono tracking-wider focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        </div>
                    @endif
                    @if ($user->email_otp_enabled)
                        <div class="flex gap-2">
                            <div class="flex-1">
                                ...
                                <input type="text" name="email_code" maxlength="6"
                                    {{ $user->email_otp_enabled ? 'required' : '' }}
                                    class="w-full border border-gray-300 rounded-lg p-3 text-center text-sm font-mono tracking-wider focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            </div>
                            <div class="flex flex-col justify-end">
                                <button type="button" onclick="sendMgmtEmailCode(this)"
                ...
                <div class="flex gap-3">
                    <button
                        class="flex-1 bg-red-700 hover:bg-red-800 px-4 py-3 rounded-lg text-white font-semibold transition-colors">
    ...
    <script>
        ...
        async function sendMgmtEmailCode(btn) {
            ...
            try {
                const res = await fetch("{{ route('2fa.management.email') }}", {
                    method: 'POST',
                    headers: {

...
<div x-data="hotkeyIndicator()" x-init="init()" 
    ...
    <button @click="toggleDisplay()" 
          class="bg-blue-300 hover:bg-blue-400 text-gray-100 border border-gray-100 rounded-full w-12 h-12 flex items-center justify-center shadow-md hover:shadow-lg transition-all duration-300 relative group">
       <i class="fas fa-question text-lg"></i>
       
       <!-- Badge with count -->
       <span x-show="hotkeyCount > 0" 
            x-text="hotkeyCount" 
            class="absolute -top-2 -right-2 bg-blue-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-bold">
       </span>
       
       <!-- Tooltip -->
       <div class="absolute bottom-full right-0 mb-3 px-3 py-2 bg-gray-800 text-white text-sm rounded-lg whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">

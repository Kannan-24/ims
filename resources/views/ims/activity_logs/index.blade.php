        ...
        <div class="w-full mx-auto max-w-7xl sm:px-6 lg:px-8 ">
            ...
            <div class="overflow-hidden bg-gray-800 rounded-lg shadow-xl p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8 mb-6">
                    ...
                    @if (!request('module') && $logs->isNotEmpty())
                        <a href="{{ route('activity-logs.destroyAll') }}"
                            onclick="return confirm('Are you sure you want to clear all activity logs?')"
                            class="px-6 py-4 rounded-xl shadow-md transition duration-300 bg-red-600 hover:bg-red-500 text-white">
                            <div class="text-lg font-bold">Clear All Logs</div>
                            <div class="text-sm text-red-200">Remove all activity logs</div>
                        </a>
                    @endif
                </div>
                ...
                <div class="flex items-center justify-between mt-4 mb-4">
                    @if (request('module'))
                        ...
                        <div class="flex gap-4">
                            ...
                            <form action="{{ route('activity-logs.destroyModule', ['module' => $mod->module]) }}"
                                method="POST" onsubmit="return confirm('Delete all logs for this module?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="text-sm bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded">

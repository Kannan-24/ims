    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function initializeCalculations() {
                ...
                setTimeout(() => {
                    calculateSummary();
                    console.log('Invoice calculations initialized successfully');
                }, 200);
            }
            ...
            document.querySelectorAll("#productTable tr").forEach(row => {
                addEventListenersToRow(row);
                // Trigger calculation for existing rows to ensure proper totals on page load
                calculateRowTotal(row);
            });
            ...
            document.querySelectorAll("#serviceTable tr").forEach(row => {
                addEventListenersToServiceRow(row);
                // Trigger calculation for existing service rows to ensure proper totals on page load
                calculateServiceRowTotal(row);
            });

            // Initial calculation - run after a brief delay to ensure all row calculations are complete
            setTimeout(() => {
                calculateSummary();
            }, 100);
            ...
            function calculateSummary() {
                try {
                    ...
                    document.querySelectorAll("#productTable tr").forEach(row => {
                        // Get values with fallbacks for proper calculation
                        let quantity = parseFloat(row.querySelector(".quantity")?.value) || 0;
                        let unitPrice = parseFloat(row.querySelector(".unit-price")?.value) || 0;
                        let cgst = parseFloat(row.querySelector(".cgst")?.value) || 0;
                        let sgst = parseFloat(row.querySelector(".sgst")?.value) || 0;
                        ...
                        if (quantity > 0 && unitPrice > 0) {
                            ...
                            let igstAmount = (baseAmount * igst) / 100;
                            let rowTotal = baseAmount + cgstAmount + sgstAmount + igstAmount;

                            productSubtotal += baseAmount;
                            productTotal += rowTotal;
                            productTotalCgst += cgstAmount;
                            productTotalSgst += sgstAmount;
                            productTotalIgst += igstAmount;

                            // Update row total if it's not already set correctly
                            const totalField = row.querySelector(".total");
                            if (totalField && Math.abs(parseFloat(totalField.value) - rowTotal) > 0.01) {
                    ...
                    document.querySelectorAll("#serviceTable tr").forEach(row => {
                        ...
                        if (quantity > 0 && unitPrice > 0) {
                            let baseAmount = quantity * unitPrice;
                            let gstAmount = (baseAmount * gstPercentage) / 100;
                            let rowTotal = baseAmount + gstAmount;

                            serviceSubtotal += baseAmount;
                            serviceTotal += rowTotal;
                            serviceTotalCgst += gstAmount / 2; // Split GST equally for CGST/SGST
                            serviceTotalSgst += gstAmount / 2;

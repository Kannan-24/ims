<x-app-layout>
            ...
            <div class="bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <form action="{{ route('invoices.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <div id="selectedCustomer"
                                class="flex items-center justify-between bg-gray-700 px-4 py-3 rounded-lg shadow-md">
                                <span id="customerName" class="text-gray-300">Select Customer</span>
                                <button type="button" id="changeCustomerButton"
                                    class="text-blue-500 hover:underline">Change</button>
                            </div>
                    ...
                    <div id="customerModal"
                        class="fixed inset-0 z-50 bg-gray-900 bg-opacity-75 hidden flex items-center justify-center">
                        <div class="bg-gray-800 p-6 rounded-lg shadow-md w-1/2">
                            <h2 class="text-2xl font-bold text-gray-200 mb-6">Select Customer</h2>
                            ...
                            <table class="min-w-full text-left table-auto bg-gray-800 text-gray-300">
                                <thead class="bg-gray-700 text-gray-400">
                                    <tr>
                                        <th class="px-6 py-4 border-b border-gray-600">Company Name</th>
                                        <th class="px-6 py-4 border-b border-gray-600">City</th>
                                ...
                                <tbody id="customerTableBody">
                                    @foreach ($customers as $customer)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 border-b border-gray-600">
                                                <button type="button"
                                                    class="select-customer px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg shadow-md transition"
                                                    data-id="{{ $customer->id }}"
                            ...
                            <button type="button" id="closeCustomerModal"
                                class="mt-4 px-6 py-3 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-lg shadow-md transition">
                                Close
                            </button>
                        </div>
                    </div>
                        ...
                        <div id="serviceSection" class="hidden">
                            ...
                            <div class="mt-6 bg-gray-700 p-4 rounded-lg shadow-md">
                                ...
                                <div class="grid grid-cols-3 gap-4">
                                    <div>
                                        <label class="block text-gray-300 font-semibold mb-2">Service
                                            Subtotal:</label>
                                ...
                                <tbody id="serviceTableBody" class="text-sm text-gray-300">
                                    @foreach ($services as $service)
                                        <tr data-id="{{ $service->id }}" class="service-row">
                                            <td class="px-6 py-4 border-b border-gray-600">{{ $service->name }}
                                            </td>
                                            <td class="px-6 py-4 border-b border-gray-600">
                                ...
                                <table class="w-full text-sm text-left text-gray-300">
                                    <tbody class="divide-y divide-gray-700">
                                        <tr>
                                            ...
                                            <td class="py-2">{{ old('service_subtotal') ?? '₹0.00' }}</td>
                                        </tr>
                                        <tr>
                                            <td class="py-2 font-medium">Service CGST Total</td>
                                            <td class="py-2">{{ old('service_total_cgst') ?? '₹0.00' }}</td>
                                        </tr>
                                        <tr>
                                            <td class="py-2 font-medium">Service SGST Total</td>
                                            <td class="py-2">{{ old('service_total_sgst') ?? '₹0.00' }}</td>
                                        </tr>
                                        <tr>
                                            <td class="py-2 font-medium">Service IGST Total</td>
                                            <td class="py-2">{{ old('service_total_igst') ?? '₹0.00' }}</td>
                                        </tr>
                                        <tr class="border-t border-gray-700 font-bold text-white">
                                            <td class="py-3">Service Total</td>
                                            <td class="py-3">{{ old('service_total') ?? '₹0.00' }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            customerSearch.addEventListener("input", function() {
                ...
                rows.forEach(row => {
                    const companyName = row.querySelector("td:nth-child(1)").textContent
                        .toLowerCase();
                    const city = row.querySelector("td:nth-child(2)").textContent.toLowerCase();

                    if (companyName.includes(searchValue) || city.includes(searchValue)) {
                        row.style.display = "";
                    } else {
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const changeCustomerButton = document.getElementById("changeCustomerButton");
            ...
            document.querySelectorAll(".select-customer").forEach(button => {
                button.addEventListener("click", function() {
                    ...
                    const customerCompanyName = this.getAttribute("data-name");

                    customerInput.value = customerId;
                    customerName.textContent = customerCompanyName;

                    // Clear and populate contact person dropdown
    ...
    <td class="p-2">
        <button type="button"
            class="open-modal bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg shadow-md transition">Select
            Product</button>
    ...
    <td class="p-2"><input type="number" name="products[${newIndex}][quantity]"
            ...
            value="1" min="1"></td>
    <td class="p-2"><input type="number" name="products[${newIndex}][unit_price]"
    ...
    <td class="p-2">
        <div class="flex items-center gap-2">
            <input type="text" name="products[${newIndex}][cgst]"
                ...
                readonly>
            <input type="text" name="products[${newIndex}][cgst_value]"
    ...
    <td class="p-2">
        <div class="flex items-center gap-2">
            <input type="text" name="products[${newIndex}][sgst]"
                ...
                readonly>
            <input type="text" name="products[${newIndex}][sgst_value]"
    ...
    <td class="p-2">
        <div class="flex items-center gap-2">
            <input type="text" name="products[${newIndex}][igst]"
                ...
                readonly>
            <input type="text" name="products[${newIndex}][igst_value]"
    ...
    <td class="p-2"><input type="text" name="products[${newIndex}][total]"
    ...
    <td class="p-2">
        <button type="button"
            class="open-service-modal bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg shadow-md transition">Select
            Service</button>
        <input type="hidden" name="services[${newIndex}][service_id]" class="service-id">
        <span class="service-name"></span>
    </td>
    <td class="p-2"><input type="number" name="services[${newIndex}][quantity]"
            ...
            value="1" min="1"></td>
    <td class="p-2"><input type="number" name="services[${newIndex}][unit_price]"
            ...
            value="0" min="0"></td>
    <td class="p-2"><input type="text" name="services[${newIndex}][gst_percentage]"
            ...
            readonly></td>
    <td class="p-2"><input type="text" name="services[${newIndex}][gst_total]"
            ...
            readonly></td>
    <td class="p-2"><input type="text" name="services[${newIndex}][total]"

                ...
                <div class="flex">
                    ...
                    <div class="ml-3">
                        ...
                        <div class="mt-2 text-sm text-yellow-700">
                            <p>This stock entry was created from a purchase order. Only quantity and price can be modified to maintain data integrity.</p>
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                <div>
                                    <label for="product_id" class="block text-sm font-medium text-gray-700 mb-2">
                                        <i class="fas fa-box text-blue-600 mr-2"></i>Product *
                                    </label>
                                    @if($stock->entry_type === 'purchase')
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">
                                            <i class="fas fa-lock mr-1"></i>Product cannot be changed for purchase entries
                                        </p>
                                    @else
                                        <select name="product_id" id="product_id" x-model="selectedProduct" @change="updateProductInfo()" 
                                ...
                                <div>
                                    <label for="supplier_id" class="block text-sm font-medium text-gray-700 mb-2">
                                        <i class="fas fa-truck text-green-600 mr-2"></i>Supplier *
                                    </label>
                                    @if($stock->entry_type === 'purchase')
                                        <div class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg">
                                            <div class="flex items-center">
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">
                                            <i class="fas fa-lock mr-1"></i>Supplier cannot be changed for purchase entries
                                        </p>
                                    @else
                                        <select name="supplier_id" id="supplier_id" x-model="selectedSupplier" @change="updateSupplierInfo()"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" required>
                                ...
                                <div>
                                    <label for="batch_code" class="block text-sm font-medium text-gray-700 mb-2">
                                        <i class="fas fa-barcode text-purple-600 mr-2"></i>Batch Code
                                    </label>
                                    <input type="text" name="batch_code" id="batch_code" value="{{ old('batch_code', $stock->batch_code) }}"
                                ...
                                <div>
                                    <label for="unit_type" class="block text-sm font-medium text-gray-700 mb-2">
                                        <i class="fas fa-balance-scale text-purple-600 mr-2"></i>Unit Type
                                    </label>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $stock->quantity) }}"
                                    ...
                                    <div>
                                        <label for="price" class="block text-sm font-medium text-gray-700 mb-2">
                                            <i class="fas fa-rupee-sign text-green-600 mr-2"></i>Price per Unit *
                                        </label>
                                        <input type="number" name="price" id="price" step="0.01" value="{{ old('price', $stock->price) }}"
                                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                ...
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-2">
                                        <i class="fas fa-tag text-gray-600 mr-2"></i>Entry Type
                                    </label>
                                    <div class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <div class="p-6 space-y-4">
                            <div class="flex items-center justify-between">
                                ...
                                <span class="text-sm font-medium text-gray-900">{{ number_format($stock->quantity) }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                <span class="text-sm text-gray-500">Currently Sold</span>
                                <span class="text-sm font-medium text-orange-600">{{ number_format($stock->sold) }}</span>
                            </div>
                            <div class="flex items-center justify-between">
                                ...
                                @endphp
                                <span class="text-sm font-medium {{ $available <= 0 ? 'text-red-600' : 'text-green-600' }}">
                                    {{ number_format($available) }}
                                </span>
                            </div>
                            <div class="flex items-center justify-between border-t pt-4">
                                <span class="text-sm text-gray-500">Total Value</span>
                                <span class="text-sm font-bold text-gray-900">₹{{ number_format($stock->quantity * $stock->price, 2) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-900 mt-1">{{ $stock->product->description }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg shadow-sm">
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-900 mt-1">{{ $stock->supplier->email }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
        ...
        <div x-show="showHelpModal" x-transition:enter="transition ease-out duration-300"
            ...
            <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                <div class="flex items-center justify-between p-6 border-b border-gray-200">
                    <h2 class="text-xl font-bold text-gray-900">Edit Stock Entry Help</h2>
                    ...
                    <div class="border border-gray-200 rounded-lg p-4">
                        ...
                        <div class="space-y-3 text-sm text-gray-600">
                            <div class="flex items-start space-x-2">
                                ...
                                <p>You can modify product, supplier, quantity, price, and sold quantity</p>
                            </div>
                            <div class="flex items-start space-x-2">
                                <i class="fas fa-exclamation-triangle text-yellow-500 mt-0.5"></i>
                                <p>Entry type (Purchase/Manual) cannot be changed after creation</p>
                            </div>
                            <div class="flex items-start space-x-2">
                                <i class="fas fa-info-circle text-blue-500 mt-0.5"></i>
                                <p>Batch code should remain unique across all stock entries</p>
                            </div>
                            <div class="flex items-start space-x-2">
                                <i class="fas fa-calculator text-purple-500 mt-0.5"></i>
                                <p>Sold quantity cannot exceed the total quantity</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <kbd class="px-2 py-1 bg-gray-100 border border-gray-300 rounded text-sm">Ctrl + R</kbd>
                                </div>
                            </div>
                            <div class="space-y-2">
                                <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <kbd class="px-2 py-1 bg-gray-100 border border-gray-300 rounded text-sm">F1</kbd>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            return {
                ...
                setupKeyboardShortcuts() {
                    document.addEventListener('keydown', (e) => {
                        if (e.ctrlKey && e.key === 's') {
                            e.preventDefault();
                            document.getElementById('stockEditForm').submit();
                ...
                resetForm() {
                    if (confirm('Are you sure you want to reset the form? All unsaved changes will be lost.')) {
                        ...
                        this.selectedProduct = '{{ $stock->product_id }}';
                        this.selectedSupplier = '{{ $stock->supplier_id }}';
                    }
                },

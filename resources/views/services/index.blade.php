            ...
            <div class="overflow-hidden bg-white rounded-lg shadow-xl">
                <div class="p-6 overflow-x-auto">
                    <table class="min-w-full text-left border-collapse table-auto">
                        ...
                        <tbody class="text-sm text-gray-700" id="serviceTable">
                            @foreach ($services as $service)
                                <tr class="border-b hover:bg-indigo-50">
                                    <td class="px-6 py-4 border-b border-gray-200">{{ $loop->iteration }}</td>
                                    <td class="px-6 py-4 border-b border-gray-200">{{ $service->name }}</td>
                                    <td class="px-6 py-4 border-b border-gray-200">{{ $service->hsn_code }}</td>
                                    <x-action-buttons :id="$service->id" :model="'services'" />
                                </tr>
                            @endforeach
                        </tbody>
                    </table>

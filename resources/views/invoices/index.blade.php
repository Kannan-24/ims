                            ...
                            @foreach ($invoices as $invoice)
                                <tr class="hover:bg-gray-700 transition duration-200">
                                    ...
                                    <td class="px-6 py-4 flex justify-center gap-3">
                                        <a href="{{ route('invoices.show', $invoice) }}" class="px-3 py-1 text-blue-400 bg-gray-800 hover:bg-gray-600 rounded-md shadow-sm transition duration-300">View</a>
                                        <a href="{{ route('invoices.edit', $invoice) }}" class="px-3 py-1 text-yellow-400 bg-gray-800 hover:bg-gray-600 rounded-md shadow-sm transition duration-300">Edit</a>
                                        <form action="{{ route('invoices.destroy', $invoice) }}" method="POST" class="inline">
                                            @csrf @method('DELETE')
                                            <button type="submit" class="px-3 py-1 text-red-400 bg-gray-800 hover:bg-gray-600 rounded-md shadow-sm transition duration-300" onclick="return confirm('Are you sure you want to delete this invoice?')">Delete</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
